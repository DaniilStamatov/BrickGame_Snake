...
Game::Game() {
  ...
  m_gameInfo.score = 0;
  m_gameInfo.level = 1;
  m_gameInfo.speed = s_speedFactor;
...
void Game::MoveSnake(float dt) {
  m_updateTime += dt;
  if(m_updateTime >= 10000) {
    m_snake.Move();
    m_updateTime = 0;
  }
  if(m_snake.IsColliding()) m_gameInfo.pause = GAME_OVER;
  if(IsCollidingFruit()) m_state = State::EATING;
}
...
bool Game::IsPositionValid(const Position &pos) const {
  std::array<Snake::SnakePart, SCALE> snakeBody = m_snake.GetSnakeBody();
